...
class CompetitorCheck(Validator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._competitors = [comp.lower() for comp in competitors]  # Convert to lowercase
        model = "en_core_web_trf"
        if self.use_local:
            self.nlp = spacy.load(model)

    def exact_match(self, text: str, competitors: List[str]) -> List[str]:
        """Performs case-insensitive exact match to find competitors from a list in a given text."""
        found_entities = []
        ...
        for entity in competitors:
            ...
            for match in matches:
                found_entities.append(text[match.start():match.end()])  # Use original case from text
        return found_entities


    def extract_entities_in_competitors_list(self, entities: List[str], competitors: List[str]) -> List:
        """Case-insensitive check if any entity from a list is present in a given list of competitors."""
        found_competitors = []
        for entity in entities:
            entity_lower = entity.lower()
            for item in competitors:
                if item == entity_lower:
                    found_competitors.append(entity)  # Use original case from entity
